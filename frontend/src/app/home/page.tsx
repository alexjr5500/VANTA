  ...
  return <div className="flex h-[100dvh] min-h-0 flex-col overflow-hidden bg-[#050505] text-white">
    ...
    <div className="w-full px-3 pb-24 pt-4">
      <main className="min-w-0">
        ...
        <section aria-label="Stories" className="mb-3 py-2"><div className="flex gap-3 overflow-x-auto scrollbar-hide">{(() => { const storyGroups = buildStoryGroups(stories); const myGroup = storyGroups.find(g => String(g.user?.id) === String(user?.id)); const mySegments = (myGroup?.stories || []).map(s => ({ id: s.id, viewed: Boolean(s.viewed) })); const myHasStories = mySegments.length > 0; return (<><div className="flex w-16 shrink-0 flex-col items-center gap-1 text-[11px] leading-tight text-[#8a8a8a]"><span className="relative block"><button type="button" aria-label={myHasStories ? 'View your statuses' : 'Add a status'} onClick={myHasStories ? () => { const opener = mySegments.find(item => !item.viewed) || mySegments[0]; if (opener) router.push(`/stories/${user?.id}?start=${opener.id}`); } : openStoryModal} className="block rounded-full"><StoryCircle src={user?.avatar} alt="Your story" active segments={myHasStories ? mySegments : undefined} /></button><button type="button" aria-label="Add a status" onClick={openStoryModal} className="absolute -bottom-1 -right-1 flex h-6 w-6 items-center justify-center rounded-full border-2 border-[#0d0d0f] bg-[#c9a227] text-black"><Plus size={14} strokeWidth={2.5}/></button></span><span>Your Story</span></div>{storyGroups.filter(g => String(g.user?.id) !== String(user?.id)).map(group => { const user0 = group.user || {}; const opener = group.hasUnviewed ? (group.stories.find(item => !item.viewed) || group.stories[0]) : group.stories[0]; if (!user0.id || !opener) return null; return <button key={user0.id} type="button" onClick={() => router.push(`/stories/${user0.id}?start=${opener.id}`)} className="flex w-16 shrink-0 flex-col items-center gap-1 text-[11px] leading-tight text-[#8a8a8a]"><StoryCircle src={user0.avatar} alt={user0.username || 'Story'} active={group.hasUnviewed} segments={(group.stories || []).map(s => ({ id: s.id, viewed: Boolean(s.viewed) }))} /><span className="max-w-16 truncate">{user0.username}</span></button>; })}</>) ; })()}</div></section>
