...
function PostCard({ item, currentUserId, onLike, onSave, onComment, onShare, onGift, onFollow, onMore }: any) {
  ...
  return <motion.article initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: .24 }} className="flex flex-col pt-3 pb-1">
    <header className="grid grid-cols-[auto_1fr] items-start gap-3 pt-2">
      <div className="grid shrink-0 gap-3 justify-self-start">
      ...
      <div className="min-w-0">
        <div className="flex min-w-0 items-center justify-between gap-x-1.5 gap-2">
          <button type="button" onClick={() => router.push(profilePath)} className="flex min-w-0 flex-1 flex-wrap items-center gap-x-1.5 text-left"><span className="min-w-0 truncate text-sm font-semibold text-[#f5f5f5]">{creator.fullName || creator.username || 'VANTA creator'}</span>{creator.verified && <VerificationBadge type="BLUE" size="sm" />}<span className="truncate text-xs text-[#666]">@{creator.username}</span><span className="text-xs text-[#666]">· {relativeTime(item.createdAt)}</span></button>
          {!isOwn && <button type="button" onClick={() => onFollow(item)} className="shrink-0 min-h-9 rounded-lg border border-white/[.12] px-3 text-xs font-semibold text-[#d8d8d8] transition hover:bg-white hover:text-black">{item.following ? 'Following' : 'Follow'}</button>}
          <button type="button" onClick={() => onMore(item)} aria-label="More post options" className="shrink-0 flex h-9 w-9 items-center justify-center rounded-lg text-[#666] transition hover:bg-white/[.05] hover:text-white"><MoreHorizontal size={19}/></button>
        </div>
        {item.location && <p className="mt-0.5 truncate text-[11px] text-[#666]">{item.location}</p>}
        {(item.content || item.description) && <p className="mb-3 mt-1 whitespace-pre-wrap text-[15px] leading-6 text-[#dedede]">{item.content || item.description}</p>}
        {item.hashtags && <p className="mb-3 text-xs text-[#c9a227]">{Array.isArray(item.hashtags) ? item.hashtags.map((tag: string) => `#${tag}`).join(' ') : item.hashtags}</p>}
      </div>
    </header>
