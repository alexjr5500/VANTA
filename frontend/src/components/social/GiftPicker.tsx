...
export default function GiftPicker({ gifts, balance, recipient, token, streamId, initialGift, loading = false, loadError, onRetry, onClose, onSent }: { gifts: GiftCatalogItem[]; balance: number; recipient: Recipient; token: string; streamId?: string; initialGift?: GiftCatalogItem | null; loading?: boolean; loadError?: string; onRetry?: () => void; onClose: () => void; onSent: (_balance: number, _amount: number, _gift: GiftCatalogItem) => void }) {
  ...
  return <>
    ...
    <motion.section
      ...
      exit={{ opacity: 0, y: '24%' }}
      className="fixed inset-x-0 bottom-[var(--vanta-kb,0px)] z-[80] mx-auto flex h-[45dvh] max-h-[min(560px,calc(var(--vanta-vh,100dvh)-140px))] w-full max-w-[480px] flex-col overflow-hidden rounded-t-[24px] border-x border-t border-white/[0.08] bg-[#101010] shadow-2xl pb-[env(safe-area-inset-bottom)] md:max-w-2xl md:h-auto md:max-h-[min(640px,calc(100dvh-160px))] md:rounded-2xl md:inset-y-0 md:my-auto md:left-1/2 md:-translate-x-1/2"
    >
