  ...
  return <header className="profile-header">
    ...
    <div className="profile-overview">
      ...
      <div className="profile-actions">
        {own ? (
          <>
            <Link className="profile-button edit-action" href="/profile/editprofile" aria-label="Edit profile"><Pencil size={16} /><span>Edit Profile</span></Link>
            <button className="profile-button share-action" onClick={onCopy} aria-label="Share profile"><Share2 size={16} /></button>
          </>
        ) : (
          <><button className={`profile-button ${profile.isFollowing ? '' : 'primary'}`} onClick={onFollow} disabled={followPending} aria-busy={followPending}>{profile.isFollowing ? <UserCheck size={16} /> : <UserPlus size={16} />}{followPending ? 'Updating' : profile.isFollowing ? 'Following' : 'Follow'}</button><button className="profile-button message" onClick={onMessage} aria-label="Message"><MessageCircle size={16} /><span>Message</span></button><button className="profile-button share-action" onClick={onCopy} aria-label="Share profile"><Share2 size={16} /></button><button className="profile-button gift-action" onClick={onGift} aria-label="Send gift"><Gift size={16} /></button></>
        )}
      </div>
