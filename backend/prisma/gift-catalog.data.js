...
exports.initialGiftCatalog = void 0;
const core = [
...
const inferArtwork = (slug) => {
    if (/fire|flame|flare|cinder|ember|phoenix/.test(slug))
        return 'flame';
    if (/rose|bloom|petal/.test(slug))
        return 'rose';
    if (/heart|love|kiss|embrace/.test(slug))
        return 'heart';
    if (/pearl|shell/.test(slug))
        return 'pearl';
    if (/medal|place|champion|league|badge|throne/.test(slug))
        return 'medal';
    if (/ride|roadster|coupe|velocity/.test(slug))
        return 'car';
    if (/jet|flight|cloud|wings|elite/.test(slug))
        return 'jet';
    if (/diamond|ice|prism|glacier|gem|crystal/.test(slug))
        return 'diamond';
    if (/crown|royalty|crest|scepter|castle/.test(slug))
        return 'crown';
    if (/happy|party|confetti|celebration/.test(slug))
        return 'happy';
    if (/thumb/.test(slug))
        return 'thumb';
    return 'artifact';
};
exports.initialGiftCatalog = [...core, ...extraGifts].map((gift, index) => (Object.assign(Object.assign({}, gift), { artworkType: gift.artworkType || inferArtwork(gift.slug), rarity: gift.price >= 2500 ? 'mythic' : gift.price >= 1000 ? 'legendary' : gift.price >= 250 ? 'rare' : 'common', tier: gift.price >= 1000 ? 'high' : gift.price >= 50 ? 'mid' : 'low', impactLevel: gift.price >= 2500 ? 5 : gift.price >= 1000 ? 4 : gift.price >= 250 ? 3 : gift.price >= 50 ? 2 : 1, effectProfile: gift.price >= 1000 ? 'cinematic-burst' : gift.price >= 250 ? 'particle-reveal' : 'polished-shimmer', sortOrder: 'sortOrder' in gift ? gift.sortOrder : index + 1 })));
//# sourceMappingURL=gift-catalog.data.js.map